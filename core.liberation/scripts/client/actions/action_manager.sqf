...
while { true } do {
	...
	if (GRLIB_player_is_menuok) then {
		...
		_idact_id = _idact_id + 1;
		_idact_num = _id_actions select _idact_id;
		if (count ([] call get_myFobs) > 0 && ( GRLIB_endgame == 0 ) && (_fobdistance < _distredeploy || _near_lhd) && (!_near_outpost) && ([player] call F_getScore >= GRLIB_perm_air ||  player == ( [] call F_getCommander ) || [] call is_admin) ) then {
			if ( _idact_num == -1 ) then {
