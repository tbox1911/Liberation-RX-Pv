...
while { true } do {
	if ([] call is_menuok) then {
		...
		if (count ([] call get_myFobs) > 0 && ( GRLIB_endgame == 0 ) && (_fobdistance < _distredeploy || (player distance my_lhd) <= 200) && ([player] call F_getScore >= GRLIB_perm_air ||  player == ( [] call F_getCommander ) || [] call is_admin) ) then {
