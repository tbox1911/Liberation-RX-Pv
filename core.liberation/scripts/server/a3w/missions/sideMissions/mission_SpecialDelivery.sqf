...
_setupObjects =
{
	...
	if (!isNil "_missionLocation") then {
		...
		_missionLocation2 = selectRandom ((west_sectors select {["capture_", _x] call fn_startsWith && (markerPos _x) distance2D (markerPos _missionLocation) > 500 && (markerPos _x) distance2D (markerPos _missionLocation) < 3000 }) apply {[_x, false]}) select 0;
		if (!isNil "_missionLocation2") then {
			...
			_missionLocation3 = selectRandom ((west_sectors select {["capture_", _x] call fn_startsWith && (markerPos _x) distance2D (markerPos _missionLocation2) > 500 && (markerPos _x) distance2D (markerPos _missionLocation2) < 3000}) apply {[_x, false]}) select 0;
