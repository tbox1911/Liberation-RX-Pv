...
private _opfor_sectors = (count sectors_allSectors) - (count west_sectors) - (count east_sectors);
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if (_opfor_factor >= 50 && GRLIB_side_enemy != INDEPENDENT) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if (count (west_sectors + east_sectors) >= 10) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if ({_x in sectors_tower} count (west_sectors + east_sectors) >= 3 && A3W_delivery_failed < 3) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if ({_x in sectors_bigtown} count (west_sectors + east_sectors) >= 1 && A3W_delivery_failed < 3) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if ({_x in sectors_factory} count (west_sectors + east_sectors) >= 3 && A3W_delivery_failed < 3) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if ({_x in sectors_military} count (west_sectors + east_sectors) >= 3 && A3W_delivery_failed < 3) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
...
if (!([_missionsList, _mission_name] call getMissionState)) then {
	if (_spawn_place >= 1 && count (west_sectors + east_sectors) >= 7 && _opfor_factor >= 50) then {
		[_missionsList, _mission_name, false] call setMissionState;
	} else {
		[_missionsList, _mission_name, true] call setMissionState;
	};
