...
while { GRLIB_csat_aggressivity > 0.9 && GRLIB_endgame == 0 } do {
	...
	_sleeptime =  (2100 + floor(random 2100)) / GRLIB_csat_aggressivity;
	...
	if ( (count allPlayers >= (10 / GRLIB_csat_aggressivity) ) && ([] call F_opforCap < GRLIB_battlegroup_cap) && (combat_readiness >= 70) && (diag_fps > 25.0))  then {
