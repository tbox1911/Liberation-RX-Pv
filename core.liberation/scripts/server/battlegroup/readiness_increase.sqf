...
while { true } do {
	if ( (count west_sectors) >= ((count sectors_allSectors) * 0.9)) then {
		if ( combat_readiness > 0 ) then {
			combat_readiness = combat_readiness - 0.15;
		};
	} else {
		if ( (combat_readiness < ((count west_sectors) * 2) && combat_readiness < 35 ) ||
			(combat_readiness < ((count west_sectors) * 1.25) && combat_readiness < 60 )) then {
