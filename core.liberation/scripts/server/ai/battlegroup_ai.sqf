...
while { ({alive _x} count (units _grp) > 0) && ( GRLIB_endgame == 0 ) } do {
	...
	_objective_sector = [100, _objective_pos, (west_sectors + east_sectors)] call F_getNearestSector;
	...
	waitUntil {sleep 5; ({alive _x} count (units _grp) == 0) || !(_objective_sector in (west_sectors + east_sectors))};
