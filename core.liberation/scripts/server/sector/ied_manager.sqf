...
if ( !(isnull _roadobj) ) then {
	...
	while { mineActive _ied_obj && !_goes_boom } do {
		_nearinfantry = [ (getpos _ied_obj) nearEntities [ "Man", _activation_radius_infantry ] , { side _x in [ GRLIB_side_west, GRLIB_side_east ] } ] call BIS_fnc_conditionalSelect;
		_nearvehicles = [ (getpos _ied_obj) nearEntities [ [ "Car", "Tank", "Air" ], _activation_radius_vehicles ] , { side _x in [ GRLIB_side_west, GRLIB_side_east ] } ] call BIS_fnc_conditionalSelect;
