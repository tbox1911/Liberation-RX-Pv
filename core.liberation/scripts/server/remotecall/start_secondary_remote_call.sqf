...
params [ "_mission_index", ["_mission_free", false], "_side" ];

_mission_cost = GRLIB_secondary_missions_costs select _mission_index;
if (_mission_free) then {_mission_cost = 0 };

if ( _mission_index == 0 ) then { [_mission_cost, _side] call fob_hunting; };
if ( _mission_index == 1 ) then { [_mission_cost, _side] call convoy_hijack; };
if ( _mission_index == 2 ) then { [_mission_cost, _side] call search_and_rescue; };
