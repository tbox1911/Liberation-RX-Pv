...
_fob_type = FOB_box_typename_east;
...
while { true } do {
	...
	if ( count _foblist == 0 && count GRLIB_fobs_east == 0 ) then {
		_fobbox = _fob_type createVehicle (getPosATL base_boxspawn_east);
		_fobbox allowdamage false;
		_fobbox setPosATL (getPosATL base_boxspawn_east);
		_fobbox setdir (getdir base_boxspawn_east);
		...
		waitUntil {
			sleep 1;
			!(alive _fobbox) || count GRLIB_fobs_east > 0
		};
		sleep 30;
