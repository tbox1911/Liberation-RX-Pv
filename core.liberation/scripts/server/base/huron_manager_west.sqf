...
while { true } do {
	...
	private _huronlist = [entities huron_typename_west, {
		(_x getVariable ["GRLIB_vehicle_ishuron", false])
	}] call BIS_fnc_conditionalSelect;

	if ( count _huronlist == 0) then {
		_huron = huron_typename_west createVehicle ( getPosATL huronspawn_west );
		...
		_huron setPosATL (getPosATL huronspawn_west);
		_huron setDir (getDir huronspawn_west);
		sleep 0.5;
		...
		if (_huron distance lhd_west < GRLIB_sector_size) then {
