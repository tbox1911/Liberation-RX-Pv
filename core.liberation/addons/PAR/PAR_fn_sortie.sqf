...
if (isPlayer _wnded) then {
  ...
  private _bounty_ok = (([(GRLIB_capture_size * 2), getPosATL _medic] call F_getNearestSector) in (sectors_allSectors - ([] call get_mySectors)) && _medic getVariable ["PAR_lastRevive",0] < time);
