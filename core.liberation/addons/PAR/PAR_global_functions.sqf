...
PAR_fn_sortie_killed = compileFinal preprocessFileLineNumbers "addons\PAR\PAR_fn_sortie_killed.sqf";
...
PAR_Player_Init = {
	...
	if (GRLIB_opfor_english && GRLIB_side_friendly == GRLIB_side_east) then {player setSpeaker "Male01ENG"};
...
PAR_HandleDamage_EH = {
	...
	if (GRLIB_tk_mode != 2) then {
		...
		if ( _isNotWounded && isPlayer _killer && _killer != _unit && side _killer == side _unit && _veh_unit != _veh_killer) then {
